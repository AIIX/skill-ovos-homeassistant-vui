...
class HomeAssistantVoiceInterface(MycroftSkill):
    def __init__(self):
        super(HomeAssistantVoiceInterface, self).__init__(
            name="HomeAssistantVoiceInterface")
        self.devices_list = None

    def initialize(self):
        self.build_device_list()
        self.bus.on(
            "ovos.phal.plugin.homeassistant.get.devices.response", self.handle_device_list)
        self.bus.on("ovos.phal.plugin.homeassistant.device.state.updated",
    ...
    def handle_turn_on_intent(self, message):
        ...
        if device:
            self.bus.emit(Message("ovos.phal.plugin.homeassistant.device.turn_on", {
                          "device_id": device_id}))
        else:
    ...
    def handle_turn_off_intent(self, message):
        ...
        device_id = self.get_device_id(device)
        if device:
            self.bus.emit(Message("ovos.phal.plugin.homeassistant.device.turn_off", {
                          "device_id": device_id}))
        else:
            self.speak_dialog("device.not.found", data={"device": device})
    ...
    def handle_set_brightness_intent(self, message):
        ...
        if device_id:
            ...
            self.bus.emit(
                Message("ovos.phal.plugin.homeassistant.call.supported.function", call_data))
        else:
            self.speak_dialog("device.not.found", data={"device": device})
...
def create_skill():
    return HomeAssistantVoiceInterface()
